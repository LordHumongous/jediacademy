...
#else
	int			(*R_Font_StrLenPixels_fp)(const char *text, const int setIndex, const float scale);
	int			(*R_Font_HeightPixels_fp)(const int setIndex, const float scale);
	void		(*R_Font_DrawString_fp)(int ox, int oy, const char *text, const float *rgba, const int setIndex, int iMaxPixelWidth, const float scale);

	int			R_Font_StrLenPixels(const char* text, const int setIndex, const float scale = 1.0f)
	{
				return R_Font_StrLenPixels_fp(text, setIndex, scale);
	}
	int			R_Font_HeightPixels(const int setIndex, const float scale = 1.0f)
	{
				return R_Font_HeightPixels_fp(setIndex, scale);
	}
	void		R_Font_DrawString(int ox, int oy, const char* text, const float* rgba, const int setIndex, int iMaxPixelWidth, const float scale = 1.0f)
	{
				R_Font_DrawString_fp(ox, oy, text, rgba, setIndex, iMaxPixelWidth, scale);
	}
#endif
